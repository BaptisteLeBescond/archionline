...
class BookingController extends Controller
{
     ...
     * Lists all booking entities.
     *
     * @Route("/country/{country}", name="booking_by_country")
     * @Method("GET")
     */
    public function bookingByCountryAction($country)
    {
        $em = $this->getDoctrine()->getManager();

        $bookings = $em->getRepository('AppBundle:Booking')->findBookingByCountry($country);
     ...
     * Lists all booking entities.
     *
     * @Route("/country/ongoing/{country}", name="ongoing_booking_by_country")
     * @Method("GET")
     */
    public function ongoingBookingByCountryAction($country)
    {
        $em = $this->getDoctrine()->getManager();

        $bookings = $em->getRepository('AppBundle:Booking')->findOngoingBookingByCountry($country);
    ...
    public function newAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $booking->setStatus(Booking::STATUS_WAITING_FOR_VALIDATION);
            $em = $this->getDoctrine()->getManager();
            $em->persist($booking);
            $em->flush();
